    ...
    return (
        <div>
            ...
            {Images.map((elem, index) => (
                <div key={index} onClick={() => handleClick(elem)}>
                    <img src={elem} alt={`Image ${index}`} style={{ width: '100px', height: 'auto' }} />
                </div>
            ))}
            {selectedImg && (
                <div>
                    <h2>Selected Image</h2>
                    <img src={selectedImg} alt="Selected" style={{ maxWidth: '500px', maxHeight: '500px' }} />
                </div>
            )}
        </div>
    )
